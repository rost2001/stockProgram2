...
public class BuyWithRobotAwt extends Order {
	...
	public BuyWithRobotAwt(Stock stock, int shares, double price, int account) {
	    this.stock = stock;
	    this.shares = shares;
	    this.price = price;
	...
	public BuyWithRobotAwt(Stock stock, double capital, double price, int account) {
	    this.stock = stock;
	...
	public void place() {
		try {
			...
			int numberOfShares = Integer.parseInt(ocr.split("Antal ")[1].split("st")[0].replace(" ", "").replace("\n", ""));
			double buyTotal = Double.parseDouble(ocr.split("SEK")[2].split("belopp")[1].replace(",", ".").replace(" ", "").replace("\n", ""));
