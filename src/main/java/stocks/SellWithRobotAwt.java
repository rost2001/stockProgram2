...
public class SellWithRobotAwt extends Order {
	...
	public SellWithRobotAwt(BuyWithRobotAwt buy, double price) {
	    this.stock = buy.stock;
	    this.shares = buy.shares;
	    this.price = price;
	    this.account = buy.account;
	}
	...
	try {
	    ...
	    stdin.println("start chrome " + "--app=\"data:text/html,<html><body><script>window.moveTo(0,0);"
		    + "window.resizeTo(800,1000);" + "window.location='https://www.avanza.se/ab/sok/inline?query="
		    + stock.symbol + "';" + "</script></body></html>\"");
	    stdin.close();
	    p.waitFor();
		...
		do {
			...
			|| !WindowsNative.getActiveWindowTitle(WindowsNative.getActiveWindow()).split("\\|")[2]
				.contains(" Avanza"));
	    ...
	    bot.keyPress(KeyEvent.VK_TAB, timeBetweenKeys);
	    bot.typeText(String.valueOf(shares), timeBetweenKeys);
	    ...
	    // Updates info from the ending popup
	    String ocr = MainOcrImage.ocr(screenshot);
		int sharesTotal = Integer.parseInt(ocr.split("Antal ")[1].split("st")[0].replace(" ", "").replace("\n", ""));
	    double sellTotal = Double.parseDouble( ocr.split("SEK")[2].split("belopp")[1].replace(",", ".").replace(" ", "").replace("\n", ""));
